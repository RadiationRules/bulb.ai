...
export const FileTree = ({ 
  ...
  const buildTree = (): FileNode[] => {
    ...
    files.forEach(file => {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (i === parts.length - 1) {
          const fileNode: FileNode = { name: part, path: file.file_path, type: 'file' };
          ...
          if (!folderMap.has(currentPath)) {
            const folderNode: FileNode = { name: part, path: currentPath, type: 'folder', children: [] };
  ...
  const handleDrop = (e: React.DragEvent, targetFolder: string) => {
    ...
    if (!sourcePath || sourcePath === targetFolder || targetFolder.startsWith(sourcePath + '/')) {
      setDraggedItem(null);
      return;
    }
    ...
    if (newPath && newPath !== sourcePath) {
      ...
      toast({ title: 'File moved', description: `${fileName} → ${targetFolder || 'root'}`, duration: 2000 });
    }
    setDraggedItem(null);
  };

  const handleDragEnd = () => { setDraggedItem(null); setDragOverFolder(null); };
  ...
  const renderNode = (node: FileNode, depth = 0) => {
    ...
    return (
      <div key={node.path}>
        <div
          className={cn(
            "group flex items-center gap-1.5 px-1.5 py-1 rounded cursor-pointer transition-all hover:bg-accent/70 text-sm",
          ...
          onClick={() => {
            if (node.type === 'folder') {
              ...
              // Single tap opens file immediately
        ...
        >
          {node.type === 'folder' ? (
            <>
              ...
              {isExpanded ? <FolderOpen className="w-3.5 h-3.5 text-bulb-glow flex-shrink-0" /> : <Folder className="w-3.5 h-3.5 text-bulb-glow flex-shrink-0" />}
            </>
          ) : (
            <>
              <div className="w-3.5" />
              <FileIcon filename={node.name} type="file" className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0" />
            </>
          )}
          ...
          {renamingItem === node.path ? (
            <input
              ...
              onBlur={() => { if (newName.trim()) handleRename(node.path, newName); else setRenamingItem(null); }}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && newName.trim()) handleRename(node.path, newName);
                else if (e.key === 'Escape') { setRenamingItem(null); setNewName(''); }
              }}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-40">
              <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setRenamingItem(node.path); setNewName(node.name); }}>
                <Edit className="w-3.5 h-3.5 mr-2" />Rename
              </DropdownMenuItem>
              <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleDelete([node.path]); }} className="text-destructive">
  ...
  const handleRootDrop = (e: React.DragEvent) => {
    ...
    if (sourcePath && sourcePath.includes('/')) {
      ...
      toast({ title: 'File moved to root', description: fileName, duration: 2000 });
    }
    setDraggedItem(null);
    setDragOverFolder(null);
  };
  ...
  return (
    <div 
      className="space-y-0 min-h-[100px]"
      onDragOver={(e) => { e.preventDefault(); if (draggedItem) setDragOverFolder('__root__'); }}
